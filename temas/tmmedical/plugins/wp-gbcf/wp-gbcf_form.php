	...
	$forms .= "\n".'<div id="gb_form_div"><!-- BEGIN: Secure and Accessible PHP Contact Form '.$form_version.' by Mike Cherim (http://green-beast.com/) and Mike Jolley (http://blue-anvil.com/) -->'."\n";
 	...
 	if ($_POST[$send_value]) {
		...
		if(!isset($posted['gbname'], $posted['email'], $posted['reason'], $posted['message'], $posted['spamq']) || empty($posted['gbname']) || empty($posted['email']) || empty($posted['reason']) || empty($posted['message']) || empty($posted['spamq'])) {
			...
			} else if($posted['spamq'] !== $options['gb_randoma']) {
				$forms.=('   <h'.$options['gb_heading'].' class="formhead" id="results">Results: <span class="error">'.$options['error_heading'].'</span></h'.$options['gb_heading'].'>
		...
		if ($options['showformhead']=='yes'){
 		...
 		$forms .= '<form id="gb_form" method="post" action="'.htmlentities($_SERVER['REQUEST_URI']).'#results">
			...
			<fieldset id="formwrap">
				<legend id="mainlegend" style="cursor:help;" title="Note: Code and markup will be removed from all fields!"><span>Contato '.$options[''].'';
		...
		if ($options['showphone']=='yes' || $options['showwebsite']=='yes') :
			$forms .= '
				<!-- Required Info -->
				<fieldset>
					<legend><span>Dados Obrigatórios:</span></legend>';
		endif;

		$forms .= '
				<label for="name">Nome</label><br /><input tabindex="'.$options['tab_name'].'" class="med" type="text" name="gbname" id="name" size="35" maxlength="40" value="" /><br /> 
				<label for="email">Email</label><br /><input tabindex="'.$options['tab_email'].'" class="med" type="text" name="email" id="email" size="35" maxlength="40" value="" /><br />';
		...
		if ($options['showphone']=='yes' || $options['showwebsite']=='yes') :
			...
			$forms .= '
				...
				<!-- Optional Info -->
				<fieldset>
					<legend><span>Dados Opcionais:</span></legend>';
			
			if ($options['showphone']=='yes') :
				$forms .= '<label for="phone">Telefone</label><br /><input tabindex="'.$options['tab_phone'].'" class="med" type="text" name="phone" id="phone" size="35" maxlength="30" value="" /><br />';
			endif;
			
			if ($options['showwebsite']=='yes') :
				$forms .= '<label for="url">Pagina na web</label><br /><input tabindex="'.$options['tab_url'].'" class="med" type="text" name="url" id="url" size="35" maxlength="60" value="http://" />';
			endif;
		...
		$forms .= '<label for="reason">Assunto</label><br /> 
		...
		$forms .= '
				...
				<label for="message">Mensagem</label><br /><textarea tabindex="'.$options['tab_message'].'" class="textbox" rows="12" cols="60" name="message" id="message"></textarea><br />
				<!-- Required anti spam confirmation -->
				<label title="No worries, the text entered here is case-insensitive" for="spamq">'.$options['gb_randomq'].'</label><br /><input tabindex="'.$options['tab_spam'].'" class="short" type="text" name="spamq" id="spamq" size="15" maxlength="30" value="" /> <small class="whythis" title="Isso confirma que voce e um ser humano.">- <a tabindex="'.$options['tab_why'].'" href="#spamq" style="cursor:help;"><span><br />
Porque isto ?</span></a></small><br />';
		...
		if($options['gb_show_cc'] == "yes") {
		    $forms .= '<input tabindex="'.$options['tab_cc'].'" class="checkbox" type="checkbox" name="gbcc" id="gbcc" value="gbcc" /> <label for="gbcc"><small>Marque esta caixa se voce quiser uma copia de e-mail.</small></label><br />'."\n"; 
		}
			
		$forms .= '<input tabindex="'.$options['tab_submit'].'" style="cursor:pointer;" class="button" type="submit" alt="Click Button to '.$options['send_button'].'" value="'.$options['send_button'].'" name="'.$send_value.'" id="'.$send_value.'" title="Enviar" />'."\n"; 
