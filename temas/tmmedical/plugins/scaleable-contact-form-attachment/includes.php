...
$scf_default_options = array(
	...
	'scf_recipient_subject' => 'Trabalhe conosco de TM Medical!',
	'scf_success_message' => 'Obrigado por suas informações!',
	...
	'scf_identifier_error' => 'Você recarregou está página? Já temos o suas informações...',
	'scf_captcha_label' => 'Por favor digite o captcha',
	'scf_submit_value' => 'Enviar solicitação',
	'scf_send_confirmation' => 'no',
	'scf_confirmation_subject' => 'Obrigado por suas informações!',
	'scf_confirmation_body' => "Olá %NAME%,\n\nObrigado por suas informações. Estaremos respondendo em breve\n\nPassar bem"
);
...
function scf_init_options_attachment( $resettable = false ) {
...
function scf_init_form_attachment( $options ) {
	...
	if ( !empty( $_POST ) && ! @empty( $_POST[ 'scf_identifier' ] ) ) {
		...
		error_log( "WITH POST, having session: '". $_SESSION[ 'scf_identifier_attachment' ]. "' and post '". $_POST[ 'scf_identifier' ]. "'" );
		
		// no valid identifier.. repost ?
		if ( @empty( $_SESSION[ 'scf_identifier' ] ) || $_POST[ 'scf_identifier' ] != $_SESSION[ 'scf_identifier' ] ) {
			error_log( "Identificador inválido" );
		...
		// captcha OK
			...
			if ( scf_check_and_send_form_attachment( $options, $_POST ) ) {
...
function scf_print_contact_form_attachment( $content = "" ) {
	global $scf_field_types;
	global $scf_identifier;
	$newCaptcha		= scf_get_captcha_instance();
	$options		= scf_init_options_attachment();
	$form			= scf_init_form_attachment( $options );
	$permalink		= get_permalink();
	
	
	$captcha_url	= scf_check_simple_captcha_attachment();
...
function scf_print_contact_form_attachment_ajax( $content = "" ) {
	$str = <<<HTML
...
HTML;
	$str .= scf_print_contact_form_attachment( $content );
	$str .= <<<HTML
	</div>
...
function scf_render_label_simple_attachment( $field ) {
	$str = <<<HTML
	<div class="form-label">
...
function scf_render_input_text_attachment( $field ) {
	$value = isset( $_POST[ 'scf_field_'. $field->name ] )
		? $_POST[ 'scf_field_'. $field->name ]
		: ''
	;
	$required = $field->required ? 'required' : '';
	$str = scf_render_label_simple_attachment($field);
	$str .= <<<HTML
...
function scf_render_textarea_attachment( $field ) {
...
function scf_render_select_attachment( $field ) {
	$value = isset( $_POST[ 'scf_field_'. $field->name ] ) ? $_POST[ 'scf_field_'. $field->name ] : '';
	$required = $field->required ? 'required' : '';
	$str = scf_render_label_simple_attachment($field);
	$str .= <<<HTML
...
function scf_render_checkbox_radio_attachment( $field ) {
	global $scf_field_types;
...
function scf_check_and_send_form_attachment( $options, $data ) {
	...
	//cabeçalho
	$headers = 'From: TM Medical - trabalhe conosco - <'.$of_mail.'>' . "\r\n";
	// now send the mail
...
function scf_check_simple_captcha_attachment() {
...
function scf_sort_fields_attachment( $a, $b ) {
	return is_object( $a )
...
function scf_get_captcha_instance_attachment() {
	global $newCaptcha;
	...
	$loadable = scf_check_simple_captcha_attachment();
	if ( $loadable === null )
		return null;
...
function scf_get_identifier_attachment( $force = false ) {
	global $scf_identifier;
